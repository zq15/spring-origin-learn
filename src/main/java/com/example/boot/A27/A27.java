package com.example.boot.A27;

import com.example.boot.A27.WebConfig;
...
//        test4(context);
        ...
        // 下面不走视图渲染的流程
        // 查看这几个处理器的 handleReturnValue 方法实现存在 mavContainer.setRequestHandled(true);
...
//        test6(context);
    ...
    private static void test5(AnnotationConfigApplicationContext context) throws Exception {
        ...
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            ...
            System.out.println(container.getModel());
            System.out.println(container.getViewName());
            if (!container.isRequestHandled()) { // 如果请求已经被处理过了，无需渲染
                renderView(context, container, webRequest); // 渲染视图
            } else {
                System.out.println(new String(response.getContentAsByteArray(), StandardCharsets.UTF_8));
            }
    ...
    public static HandlerMethodReturnValueHandlerComposite getReturnValueHandler() {
        ...
        composite.addHandler(new ModelAndViewMethodReturnValueHandler()); // test1 ModelAndView
        composite.addHandler(new ViewNameMethodReturnValueHandler()); // test2 视图名
        composite.addHandler(new ServletModelAttributeMethodProcessor(false)); // test3 @ModelAttribute 返回值对象作为 ModelAndView
        ...
        composite.addHandler(new ServletModelAttributeMethodProcessor(true)); // test4
        return composite;
    }
